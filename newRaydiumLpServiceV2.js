...
const JUPITER_ADDRESS = new PublicKey('JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4');
...
// Function to check if the transaction involves Jupiter's address
function isTransactionFromJupiter(transactionDetails) {
    const message = transactionDetails.transaction.message;
    return message.accountKeys.some(key => key.equals(JUPITER_ADDRESS));
}
...
async function processRaydiumLpTransaction(connection, signature) {
    try {
        ...
        if (transactionDetails) {
            // Check if the transaction involves Jupiter's address
            if (isTransactionFromJupiter(transactionDetails)) {
                console.log(`Ignoring transaction involving Jupiter: ${signature}`);
                return;
            }
